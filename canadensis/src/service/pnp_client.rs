...
use crate::{Node, PublishToken, TransferHandler};
use alloc::vec::Vec;
    ...
    M: AllocationMessage<N::Transport>,
{
    ...
    pub fn send_request(&mut self, node: &mut N) -> Result<(), ()> {
        let message = M::with_unique_id(&self.unique_id);
        let status = node.publish(self.publish_token.as_ref().ok_or(())?, &message);
        match status {
            Ok(()) => Ok(()),
