...
{
    pub fn new(mut node: Node<C, Q, P, R>) -> Result<Self, CapacityError> {
        ...
        let heartbeat_timeout = <C::Instant as Instant>::Duration::from_millis(500)
    ...
    pub fn node(&self) -> &Node<C, Q, P, R> {
        &self.node
    }

    /// Returns a mutable reference to the enclosed node
    pub fn node_mut(&mut self) -> &mut Node<C, Q, P, R> {
        &mut self.node
    }
}
