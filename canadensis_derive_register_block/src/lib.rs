...
/// Implements `canadensis_node::register::RegisterBlock` for a struct that contains zero or more
/// register fields
///
/// Each field in the struct must implement `canadensis_node::register::Register`.
...
fn implement_register_block(input: DeriveInput) -> proc_macro2::TokenStream {
    ...
    quote! {
        impl ::canadensis_node::register::RegisterBlock for #struct_name {
            fn register_by_index(&self, index: usize) -> Option<&dyn ::canadensis_node::register::Register> {
                match index {
                    #( #field_indices => Some(&self.#field_names), )*
                    _ => None,
                }
            }
            fn register_by_index_mut(&mut self, index: usize) -> Option<&mut dyn ::canadensis_node::register::Register> {
                match index {
                    #( #im_field_indices => Some(&mut self.#im_field_names), )*
                    _ => None,
                }
            }
            fn register_by_name_mut(&mut self, name: &str) -> Option<&mut dyn ::canadensis_node::register::Register> {
                match name {
                    #( name if name == canadensis_node::register::Register::name(&self.#name_field_names) => Some(&mut self.#name_field_names), )*
                    _ => None,
                }
            }
        }
